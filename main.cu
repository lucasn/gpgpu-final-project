...
double accuracy(image* test_img, byte* test_label, unsigned datasize, unsigned minibatch_size, ann_t *nn)
{
    ...
    CHECK_ERROR(cudaMallocManaged(&x, 28 * 28 * minibatch_size * sizeof(double)));
    CHECK_ERROR(cudaMallocManaged(&y, 10 * minibatch_size * sizeof(double)));
...
int main(int argc, char *argv[])
{
    ...
    CHECK_ERROR(cudaMallocManaged(&shuffled_idx, datasize*sizeof(unsigned)));
