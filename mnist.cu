...
byte* read_labels(const char filename[], unsigned* n )
{
    ...
    *n = make_uint32(buf);
    ...
    CHECK_ERROR(cudaMallocManaged(&ls, (*n) * sizeof(byte)));
    CHECK_ERROR(cudaMemset(ls, 0, (*n) * sizeof(byte)));
...
image* read_images(const char filename[], unsigned* n )
{
    ...
    *n = make_uint32(buf);
    ...
    CHECK_ERROR(cudaMallocManaged(&is, (*n) * sizeof(image)));
    CHECK_ERROR(cudaMemset(is, 0, (*n) * sizeof(image)));
